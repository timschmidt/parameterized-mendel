...
<mendel_misc_2head.inc>

x_car_top_height=14;
x_car_width=44;
x_top_knob_height=2;
x_top_knob_length=13;
x_centre_cavity_length=128;//73;
...
module x_car_top_block() {
	union () {
		...
		translate([(x_car_length-x_top_knob_length)/-2,0,x_car_top_height+x_top_knob_height/2])
			box(x_top_knob_length,x_car_top_knob_width,x_top_knob_height);
		// WIngs
		translate ([33,(wing_width+x_car_width)/2,wing_height/2]) box(wing_length,wing_width,wing_height);
		translate ([35.5,(wing_width+x_car_width)/-2+1.5,wing_height/2]) difference () {
...
module cutouts() {
	union () {
		...
		translate ([-x_car_wing_offset,0,10]) rotate([180,0,30]) 
			cylinder(r=21,h=11);
		translate([-x_car_wing_offset,0,0]) rotate([0,0,0]) triangle(42,30);
